...
try:
    ...
    if isinstance(labels_raw, dict):
        labels = [
            labels_raw[k]
            for k in sorted(labels_raw, key=lambda x: int(x))
        ]
    else:
        labels = labels_raw
    ...
    REMEDIES = {
        "Leaf Blight": {
            ...
            "ta": (
                ...
                "மேலிருந்து நீர் பாய்ச்சுவதை தவிர்த்து, "
        ...
        "Powdery Mildew": {
            "en": (
                "Use neem oil spray as a general treatment. "
                "Ensure good sunlight and avoid overcrowding."
            ),
            "ta": (
                "வேப்பெண்ணெய் தெளிப்பை பயன்படுத்தவும். "
                "நல்ல சூரிய ஒளி கிடைப்பதை உறுதி செய்யவும்."
            ),
        },

        "Bacterial Spot": {
            "en": (
                "Use disease-free seeds and maintain good field hygiene. "
                "Consult an agricultural expert for severe infection."
            ),
            "ta": (
                "நோய் இல்லாத விதைகளை பயன்படுத்தவும். "
                "கடுமையான பாதிப்பிற்கு வேளாண் நிபுணரை அணுகவும்."
            ),
        },
        ...
        "Leaf Rust": {
            ...
            "ta": (
                "அதிகம் பாதிக்கப்பட்ட இலைகளை அகற்றி "
                "காற்றோட்டத்தை மேம்படுத்தவும்."
            ),
        },
    }
    ...
    def get_remedy(disease, lang):
        ...
        if not entry:
            return (
                ...
                if lang == "en"
                else "உங்கள் உள்ளூர் வேளாண் அலுவலரை அணுகவும்."
            )
...
def answer_query(message, lang):
    ...
    if any(
        w in text
        for w in ["water", "irrigat", "நீர்", "பாசனம்"]
    ):

        if s["soil_status"] == "DRY":
            return r(
                "The soil is currently DRY. "
                "I recommend irrigation. "
                "You can turn the pump ON from the Farm Sensors tab.",
                "மண் தற்போது உலர்ந்த நிலையில் உள்ளது. "
                "பாசனம் செய்ய பரிந்துரைக்கிறேன். "
                "Farm Sensors பகுதியில் இருந்து பம்பை ON செய்யலாம்.",
            )

        return r(
            "The soil is currently WET. "
            "Irrigation may not be necessary right now.",
            "மண் தற்போது ஈரமாக உள்ளது. "
            "இப்போது பாசனம் தேவையில்லை.",
        )

    if any(
        w in text
        for w in ["temperature", "hot", "cold", "வெப்ப", "வெப்பநிலை"]
    ):
    ...
    if any(
        w in text
        for w in ["fertiliz", "manure", "உரம்"]
    ):

        return r(
            "Use a balanced fertilizer according to your crop's growth stage. "
            "Organic manure can also improve soil health.",
            "பயிரின் வளர்ச்சி நிலைக்கு ஏற்ப சமச்சீர் உரத்தை பயன்படுத்தவும். "
            "இயற்கை உரம் மண் ஆரோக்கியத்தை மேம்படுத்தும்.",
        )

    if any(
        w in text
        for w in ["pest", "insect", "bug", "பூச்சி"]
    ):

        return r(
            "For general pest control, neem oil can be considered. "
            "For severe infestation, identify the pest before using pesticides.",
            "பொதுவான பூச்சி கட்டுப்பாட்டிற்கு வேப்பெண்ணெய் பயன்படுத்தலாம். "
            "கடுமையான தாக்குதலில் பூச்சியை அடையாளம் கண்ட பிறகு மருந்தை பயன்படுத்தவும்.",
        )
...
<link href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700;800&family=Noto+Sans+Tamil:wght@400;500;600;700&display=swap"
      rel="stylesheet">
...
:root {
    ...
    --danger:#e2574c;
    --radius:18px;
    --shadow:0 10px 30px rgba(31,122,68,.10);
}
...
id="tabDetectBtn"
onclick="showTab('detect')">
📷 <span id="tabDetectLabel">
Detect Disease
</span>
</button>
...
class="tab-btn"
id="tabSensorsBtn"
onclick="showTab('sensors')">
🌡️ <span id="tabSensorsLabel">
Farm Sensors
</span>
</button>
...
type="text"
id="chatInput"
placeholder="Ask about watering, pests, fertilizer..."
...
chatPlaceholder:
"Ask about watering, pests, fertilizer...",
...
uploadTitle:
"📸 உங்கள் பயிரின் படத்தை பதிவேற்றவும்",

uploadText:
"பாதிக்கப்பட்ட இலையின் தெளிவான படத்தை தேர்ந்தெடுக்கவும்",

uploadBtn:"📤 படத்தைத் தேர்ந்தெடுக்கவும்",

detectBtn:
"🔎 ஃபார்மியிடம் பகுப்பாய்வு கேளுங்கள்",

loadingText:"பகுப்பாய்வு செய்கிறது...",

chatPlaceholder:
"நீர்ப்பாசனம், பூச்சி, உரம் பற்றி கேளுங்கள்...",

sensorHeading:
"🌡️ நேரடி பண்ணை நிலைமைகள்",
...
selectImage:
"முதலில் ஒரு படத்தை தேர்ந்தெடுக்கவும்.",
...
function setLang(l) {
    ...
    document
        ...
        .classList
        .toggle("active", l === "english");

    document
        .getElementById("langTaBtn")
        .classList
        .toggle("active", l === "tamil");

    document.getElementById("brandTitle")
        .textContent = t.brandTitle;

    document.getElementById("brandSubtitle")
        .textContent = t.brandSubtitle;

    document.getElementById("tabDetectLabel")
        .textContent = t.tabDetect;

    document.getElementById("tabChatLabel")
        .textContent = t.tabChat;

    document.getElementById("tabSensorsLabel")
        .textContent = t.tabSensors;

    document.getElementById("uploadTitle")
        .textContent = t.uploadTitle;

    document.getElementById("uploadText")
        .textContent = t.uploadText;

    document.getElementById("uploadBtn")
        .textContent = t.uploadBtn;

    document.getElementById("detectBtn")
        .textContent = t.detectBtn;

    document.getElementById("loadingText")
        .textContent = t.loadingText;

    document.getElementById("chatInput")
        .placeholder = t.chatPlaceholder;

    document.getElementById("sensorHeading")
        .textContent = t.sensorHeading;

    document.getElementById("refreshLabel")
        .textContent = t.refreshLabel;
...
async function detectDisease() {
    ...
    try {
        ...
        document
            .getElementById("farmieMsg")
            .textContent = data.farmie_msg;
...
function toggleMic() {
    ...
    mediaRecognition.lang =
        lang === "tamil"
            ? "ta-IN"
            : "en-US";

    mediaRecognition.interimResults =
        false;

    mediaRecognition.maxAlternatives =
        1;
...
function renderSensors() {
    ...
    document
        ...
        <div class="sensor-card">
            ...
            <div class="value">
                ${
                    sensorData.temperature_c !== null
                    ? sensorData.temperature_c + "°C"
                    : "--"
                }
            </div>
    ...
    document
        .getElementById("irrigationLabel")
        .textContent =
        sensorData.irrigation_on
        ? t.irrigationOn
        : t.irrigationOff;
    ...
    if (sensorData.last_updated > 0) {
        ...
        const date =
            new Date(
                sensorData.last_updated * 1000
            );

        document
            .getElementById("updatedNote")
            .textContent =
            t.updated +
...
def _run_detect():
    ...
    if healthy:
        ...
        if language == "tamil":
            ...
            title =
                "✅ வாழ்த்துக்கள்! ஆரோக்கியமாக உள்ளது!"
        ...
        if language == "tamil":
            ...
            title =
                f"⚠️ பயிர் நோய்: {predicted_class}"
            ...
            farmie_msg =
                f"🌱 ஃபார்மி: {predicted_class} கண்டறியப்பட்டுள்ளது."

        else:

            title =
                f"⚠️ Crop Disease Detected: {predicted_class}"
            ...
            farmie_msg =
                f"🌱 Farmie: {predicted_class} detected."
